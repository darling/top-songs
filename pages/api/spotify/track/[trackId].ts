...
const handler = async (
	...
	const apiResponse = await axios.get<SpotifyApi.TrackObjectFull>(
		'https://api.spotify.com/v1/tracks/' + req.query.trackId,
		{
			headers: {
	...
	return res.status(200).json({ profile: apiResponse.data });
};
